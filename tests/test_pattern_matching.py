...
def test_pattern_matching():
    ...
    assert _matches_protected_pattern('PEG-400', frozenset({'PEG_400'})) == True, 'PEG-400 should match PEG_400 after canonicalization'
    assert _matches_protected_pattern('PEG_400', frozenset({'PEG-400'})) == True, 'PEG_400 should match PEG-400 after canonicalization'
    assert _matches_protected_pattern('TFSI', frozenset({'TFSI'})) == True, 'TFSI exact match should work'
    assert _matches_protected_pattern('TFSI-1', frozenset({'TFSI'})) == True, 'TFSI-1 should match TFSI boundary'
    ...
    assert _matches_protected_pattern('LIPID', frozenset({'LI'})) == False, 'LI should NOT match LIPID'
